...
def config():
    return NS(
        ...
        head_dim=4, attention_hidden_size=8, value_dim=[8,8],
        decay_low_rank_dim=3, a_low_rank_dim=3,
        gate_low_rank_dim=4, v_low_rank_dim=2,
    )

def test_one_token_block_matches_canonical_hf_oracle():
    torch.manual_seed(123)
    c=config(); mc=NS(dtype=torch.float32); cc=NS(enable_prefix_caching=False)
    block=RWKV7Block(c,mc,cc,1,"model.layers.1")
    for p in block.parameters(): p.data.normal_(0,0.05)
    x=torch.randn(8); att_prev=torch.randn(8); ffn_prev=torch.randn(8)
    v_first=torch.randn(8); state=torch.randn(2,4,4,dtype=torch.float32)
