...
def test_join_exit_reorder_compact_and_slot_reuse():
    ...
    c.wkv[:, a].fill_(1); c.wkv[:, b].fill_(2); c.wkv[:, d].fill_(3)
...
def test_cancel_release_is_fail_closed_and_zeroizes():
    c = cache(1); s = c.allocate("request")
    c.att_x[:,s].fill_(7); c.release(s)
    assert torch.count_nonzero(c.att_x) == 0
    with pytest.raises(KeyError): c.select([s])
    with pytest.raises(KeyError): c.release(s)

def test_fork_offload_restore_and_chunk_continuation():
    c = cache(); s = c.allocate("root")
    c.seen_tokens[s] = 128; c.wkv[:,s].normal_()
    child = c.clone_or_fork(s, "child")
    assert torch.equal(c.wkv[:,s], c.wkv[:,child])
    snapshot = c.offload(child)
    c.wkv[:,child].zero_(); c.restore(child, snapshot)
    assert torch.equal(c.wkv[:,s], c.wkv[:,child])
...
def test_capacity_duplicate_and_bad_reorder_rejected():
    c = cache(1); c.allocate("a")
    with pytest.raises(ValueError): c.allocate("a")
    with pytest.raises(RuntimeError): c.allocate("b")
    with pytest.raises(ValueError): c.reorder([0], [1])
