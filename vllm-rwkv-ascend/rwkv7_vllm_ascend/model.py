...
class RWKV7Attention(nn.Module):
    def __init__(self, config, layer_idx: int, hidden: int, heads: int, head_dim: int):
        ...
        self.v_lora = _LoRA(hidden, _rank(config, "v", 32), attention_hidden, True) if layer_idx > 0 else None
...
class RWKV7FFN(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.key = quant_key if quant_key is not None else RawLinear(intermediate, hidden)
        self.value = quant_value if quant_value is not None else RawLinear(hidden, intermediate)
...
class RWKV7Block(MambaBase, nn.Module):
    def __init__(
        ...
        head_size = int(_attr(config, "head_dim", "head_size", "head_size_a", default=64))
        value_dims = getattr(config, "value_dim", None)
        attention_hidden = int(_attr(config, "attention_hidden_size", default=(value_dims[layer_idx] if isinstance(value_dims, (list, tuple)) else hidden)))
        ...
        intermediate = int(_attr(config, "intermediate_size", "dim_ffn", default=hidden * 4))
        self.hidden, self.heads, self.head_size, self.attention_hidden = hidden, heads, head_size, attention_hidden
    ...
    def get_state_shape(self):
        return ((self.heads, self.head_size, self.head_size), (self.hidden,), (self.hidden,))
    ...
    def _token_recurrence(self, x, state, att_prev, ffn_prev, v_first):
        ...
        xr = self._mix(xx, att_prev, a.x_r.view(-1)); xw = self._mix(xx, att_prev, a.x_w.view(-1))
        xk = self._mix(xx, att_prev, a.x_k.view(-1)); xv = self._mix(xx, att_prev, a.x_v.view(-1))
        xa = self._mix(xx, att_prev, a.x_a.view(-1)); xg = self._mix(xx, att_prev, a.x_g.view(-1))
        r = a.r_proj(xr); k = a.k_proj(xk); v = a.v_proj(xv)
        ...
        w = torch.exp(-EXP_HALF * torch.sigmoid(w_raw)).view(self.heads, 1, self.head_size)
        ...
        y = (state.to(r.dtype) @ rh.unsqueeze(-1)).squeeze(-1).reshape(self.attention_hidden)
        y = F.group_norm(y.view(1, self.attention_hidden, 1), self.heads, a.g_norm.weight, a.g_norm.bias, a.g_norm.eps).view(self.attention_hidden)
        sk = (rh * kh * a.r_k).sum(-1, keepdim=True)
        y = (y.view(self.heads, self.head_size) + sk * vh).reshape(self.attention_hidden) * g
    ...
    def _segments(self, metadata, token_count: int):
        ...
        if nd_tokens != nd:
            raise NotImplementedError(
                "RWKV-7 speculative decode is not supported; expected one token per decode request")
        ...
        if trace_path and getattr(self, "layer_idx", -1) == 0:
            trace = {
                "num_decodes": nd, "num_decode_tokens": nd_tokens,
                "num_prefills": npref,
                "num_prefill_tokens": int(metadata.num_prefill_tokens),
                "decode_state_slots": (decode_ids.detach().cpu().tolist()
                                       if decode_ids is not None else None),
                "prefill_state_slots": (prefill_ids.detach().cpu().tolist()
                                        if prefill_ids is not None else None),
                "query_start_loc_p": (metadata.query_start_loc_p.detach().cpu().tolist()
                                      if metadata.query_start_loc_p is not None else None),
                "has_initial_states_p": (metadata.has_initial_states_p.detach().cpu().tolist()
                                         if metadata.has_initial_states_p is not None else None),
            }
    ...
    def forward(self, hidden_states: torch.Tensor, v_first: torch.Tensor | None):
        ...
        vf_output = hidden_states.new_zeros((hidden_states.shape[0], self.attention_hidden)) if self.layer_idx == 0 else v_first
        if all_metadata is None:
            state = torch.zeros(self.heads, self.head_size, self.head_size, dtype=torch.float32, device=hidden_states.device)
            ap = torch.zeros(self.hidden, dtype=hidden_states.dtype, device=hidden_states.device)
            fp = torch.zeros_like(ap)
            for pos in range(hidden_states.shape[0]):
                vt = None if v_first is None else v_first[pos]
                output[pos], state, ap, fp, vo = self._token_recurrence(hidden_states[pos], state, ap, fp, vt)
                if self.layer_idx == 0: vf_output[pos] = vo
        ...
        if self.cache_config.enable_prefix_caching:
            raise NotImplementedError("RWKV-7 prefix caching is disabled until block-boundary state parity is validated")
        ...
        for begin, end, slot, fresh in self._segments(metadata, hidden_states.shape[0]):
            if fresh:
                stale_nonzero = None
                if trace_path := os.getenv("RWKV7_TRACE_PATH"):
                    if self.layer_idx == 0:
                        stale_nonzero = bool(
                ...
                if stale_nonzero is not None:
                    zero_event = {
                        "event": "fresh_state_zero", "slot": slot,
                ...
                state = wkv_cache[slot]; ap = att_cache[slot]; fp = ffn_cache[slot]
            for pos in range(begin, end):
                vt = None if v_first is None else v_first[pos]
                output[pos], state, ap, fp, vo = self._token_recurrence(hidden_states[pos], state, ap, fp, vt)
                if self.layer_idx == 0: vf_output[pos] = vo
            wkv_cache[slot].copy_(state); att_cache[slot].copy_(ap); ffn_cache[slot].copy_(fp)
        return output, vf_output
...
class RWKV7Model(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList([
            RWKV7Block(
                ...
                i,
                f"{prefix}.layers.{i}.linear_attn" if prefix
    ...
    def forward(self, input_ids, positions, intermediate_tensors: IntermediateTensors | None = None, inputs_embeds=None):
        if intermediate_tensors is not None:
            raise NotImplementedError("pipeline parallelism is not supported by the RWKV-7 plugin")
...
class RWKV7ForCausalLM(nn.Module, HasInnerState, IsAttentionFree):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if os.getenv("RWKV7_ALLOW_UNVALIDATED_ASCEND") != "1":
            try:
                ...
                raise RuntimeError(
                    "RWKV-7 vLLM plugin requires the validated Ascend 910B3 device") from exc
        ...
        if vllm_config.parallel_config.tensor_parallel_size != 1 or vllm_config.parallel_config.pipeline_parallel_size != 1:
            raise NotImplementedError("RWKV-7 Ascend plugin currently requires TP=1 and PP=1")
        ...
        self.lm_head = ParallelLMHead(
            int(config.vocab_size), hidden, bias=False,
            params_dtype=vllm_config.model_config.dtype,
    ...
    def get_mamba_state_shape_from_config(cls, vllm_config):
        ...
        attention_hidden = int(_attr(c, "attention_hidden_size", default=(value_dims[0] if isinstance(value_dims, (list, tuple)) else hidden)))
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for original_name, tensor in weights:
            ...
            if name.startswith("blocks."): name = "model." + name
            if self._ascend_quant is not None:
            ...
            if name not in params:
                LOG.debug("ignoring checkpoint tensor not owned by vLLM model: %s", original_name)
                continue
            param = params[name]
            value = tensor
            ...
            if value.shape != param.shape:
                raise ValueError(f"shape mismatch for {original_name} -> {name}: checkpoint {tuple(value.shape)}, model {tuple(param.shape)}")
            loader = getattr(param, "weight_loader", default_weight_loader)
            loader(param, value)
            loaded.add(name)
        missing = sorted(set(params) - loaded)
        if missing:
            raise ValueError(f"RWKV-7 checkpoint is missing {len(missing)} parameters; first: {missing[:8]}")
        if self._ascend_quant is not None:
