...
class RwkvLayerNormDirectKernel {
public:
...
extern "C" __global__ __aicore__ void rwkv_layer_norm_direct(
    GM_ADDR x,
    GM_ADDR weight,
    GM_ADDR bias,
    GM_ADDR out,
    ...
    float inv_n,
    uint32_t elements) {
    RwkvLayerNormDirectKernel kernel;
    kernel.Init(x, weight, bias, out, epsilon, inv_n, elements);
    kernel.Process();
}
