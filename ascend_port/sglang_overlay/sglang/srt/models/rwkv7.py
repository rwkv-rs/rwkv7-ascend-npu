...
# Ascend port: the rwkv7_kernels (CUDA/Triton) modules are absent on NPU. Import
# them optionally; stub with .available()==False so the model loads and runs the
# plain-torch projection paths (M1c token-exact). The fused/quant/LoRA fast paths
# are gated by env flags (default off) so the stubs are never called for base fp16.
try:
