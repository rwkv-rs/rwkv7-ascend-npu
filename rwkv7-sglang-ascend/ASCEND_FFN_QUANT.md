...
## SGLang loader wiring

`Rwkv7ForCausalLM.load_weights` intercepts only the selected canonical
`model.layers.N.ffn.{key,value}` names. All other parameters and legacy loader
paths are unchanged. Native Ascend FFN quantization is mutually exclusive with
SGLang `quant_config` and the legacy CUDA `RWKV_W4`/`RWKV_W8` flags.
