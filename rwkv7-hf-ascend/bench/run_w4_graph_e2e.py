#!/usr/bin/env python3
"""Real 7.2B HF NPUGraph diagnostic for the public Ascend W4A16 candidate API.
...
This diagnostic tests value-only group-128 affine W4 on the exact device, stack,
FP16 dtype, 7.2B FFN shape and B1/B4/B8 logical rows. It cannot populate the
production policy unless every quality, memory and latency gate passes.
"""
...
QUALITY_CASES = (
    ...
    (
        "holdout_fact",
        (33329, 4600, 22590, 51128, 4706, 44312, 47),
    ),
    (
        "holdout_chinese",
        (16738, 14589, 10250, 11012, 16713, 16533, 17382, 17387, 11855, 16664, 15200, 10080),
    ),
    (
        "holdout_testing",
        (6654, 8150, 7946, 22763, 32343, 39923, 46530, 4596, 57257, 63528, 47),
    ),
    (
        "holdout_sql",
        (33586, 332, 20826, 39415, 32227, 45600, 40084, 4450, 51337, 47),
    ),
    (
        "holdout_networking",
        (23618, 6957, 22590, 61863, 51035, 20952, 21265, 21046, 47),
    ),
)
...
def main() -> None:
    ...
    replaced = quantize_ascend_w4a16_candidate(
        quant,
        group_size=128,
        roles=("ffn.value",),
        require_explicit_candidate=False,
    )
    quant_report = {"policy": "candidate", "replaced": replaced}
    ...
    gates = {
        ...
        "all_32_ffn_value_projections_replaced": len(replaced) == 32,
    ...
    result = {
        "schema": "rwkv7-ascend-hf-w4-value-native-graph-e2e-v1",
        "status": "PASS" if all(gates.values()) else "FAIL",
        "scope": "real-checkpoint-candidate-diagnostic",
        "environment": {
    ...
    print("ASCEND_W4_GRAPH_JSON", json.dumps(result, ensure_ascii=False))
    if result["status"] != "PASS":
        raise SystemExit(1)
    print("ASCEND_W4_GRAPH_PASS")
