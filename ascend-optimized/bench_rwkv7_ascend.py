"""Correctness + perf for v3 C++ decode_full (with norms + LoRA biases) on Ascend."""
import os
...
mod3 = load(name="rwkv7_ascend_v3", sources=[os.path.join(os.path.dirname(os.path.abspath(__file__)), "rwkv7_ascend.cpp")],
            verbose=False, extra_cflags=["-O3","-std=c++17"])
base=model.model; L=NLAYERS; B=1; hidden=H*N
...
def cpp3(sa,xp,xf,vf):
    return mod3.rwkv7_decode_full(emb,rw,kw,vw,ow,fkw,fvw,w0l,w2l,a0l,a2l,g0l,g2l,v0l,v2l,
        w2b,a2b,v2b,xr,xw,xk,xv,xa,xg,kk_l,ka_l,rk_l,gnw,gnb,fxk,
        anw,anb,fnw,fnb,pnw,pnb,sa,xp,xf,vf,H,N,lm_w,fnorm_w,fnorm_b)

with torch.no_grad():
    py_logits=model(ids).logits[0,-1].float().cpu()
    try:
        c3=cpp3(*fresh_state())[0].float().cpu()
    except Exception as e:
        c3=None; print("[v3 FAILED: %s: %s]"%(type(e).__name__,str(e)[:300]),flush=True)

if c3 is not None:
    print("CORRECTNESS (%d layers) vs Python:"%NLAYERS,flush=True)
    print("  py finite=%s cpp finite=%s"%(bool(torch.isfinite(py_logits).all()),bool(torch.isfinite(c3).all())),flush=True)
    cos=F.cosine_similarity(py_logits.unsqueeze(0),c3.unsqueeze(0)).item()
    mx=(py_logits-c3).abs().max().item()
    print("  cos=%.5f maxabs=%.4f argmax_match=%d (py=%d cpp=%d)"%(cos,mx,int(py_logits.argmax()==c3.argmax()),int(py_logits.argmax()),int(c3.argmax())),flush=True)
...
print("\nPERF (fp16, B=1, %d layers):"%NLAYERS,flush=True)
print("  Python : %.2f ms = %5.0f tok/s"%(py_ms,1000/py_ms),flush=True)
if c3 is not None:
    c3_ms=bench(lambda: cpp3(*fresh_state()))
    print("  C++ v3 : %.2f ms = %5.0f tok/s (%.2fx vs py)"%(c3_ms,1000/c3_ms,py_ms/c3_ms),flush=True)
