"""Run the profiler and print per-section timing."""
import os
...
_npu_dir=os.path.dirname(torch_npu.__file__)
mod=load(name="rwkv7_prof",sources=["/root/rwkv7_prof.cpp"],verbose=False,
         extra_cflags=["-O3","-std=c++17"],
         extra_include_paths=[os.path.join(_npu_dir,"include")],
         extra_ldflags=[f"-L{os.path.join(_npu_dir,'lib')}", "-ltorch_npu", f"-Wl,-rpath,{os.path.join(_npu_dir,'lib')}"])
...
# warmup
for _ in range(3):
    r=mod.rwkv7_prof(emb,rw,kw,vw,ow,fkw,fvw,w0l,w2l,a0l,a2l,g0l,g2l,v0l,v2l,w2b,a2b,v2b,xr,xw,xk,xv,xa,xg,kk_l,ka_l,rk_l,gnw,gnb,fxk,anw,anb,fnw,fnb,pnw,pnb,sa,xp,xf,vf,H,N)
# take last run's numbers
labels=["norms","shiftmix","proj_rkv","loras","kk+vmod","wkv","gn+sk+o","ffn"]
print("Per-section (total us / per-layer us):",flush=True)
tot=0
for i,name in enumerate(labels):
    tot_us=r[2*i].item(); per=r[2*i+1].item(); tot+=tot_us
    print("  %-10s %7.1f us total  %5.1f us/layer"%(name,tot_us,per),flush=True)
print("  TOTAL      %7.1f us  (%.2f ms)"%(tot,tot/1000),flush=True)
